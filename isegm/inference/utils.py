...
from isegm.data.datasets import GrabCutDataset, BerkeleyDataset, DavisDataset, SBDEvaluationDataset, PascalVocDataset,RetinalDataset, Retinal_Auto_Mask_Dataset
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
        ...
        dataset = RetinalDataset(cfg.RETINAL_PATH)
    ...
    elif dataset_name == 'RETINAL_COARSE_VAL':
        dataset = Retinal_Auto_Mask_Dataset(cfg.RETINAL_VAL_PATH)
    else:
        dataset = None
...
def compute_noc_metric(all_ious, iou_thrs, max_clicks=30):
    def _get_noc(iou_arr, iou_thr):
        vals = iou_arr >= iou_thr
        vals1 = vals.astype(int)
        return max_clicks - np.sum(vals1) if np.any(vals) else max_clicks
    noc_list = []
    over_max_list = []
...
def get_results_table(noc_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time,
    ...
    table_header = (f'|{"BRS Type":^13}|{"Dataset":^11}|'
                    f'{"NoC@65%":^9}|{"NoC@70%":^9}|{"NoC@75%":^9}|'
                    f'{">="+str(n_clicks)+"@70%":^9}|{">="+str(n_clicks)+"@75%":^9}|'
                    f'{"SPC,s":^7}|{"Time":^9}|')
